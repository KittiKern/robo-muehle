...
Currently, the model is implemented as a simple feedforward network. However, since the game of Nine Men’s Morris can be represented as a graph and capturing graph-based relationships may contribute to a better modeling of the game structure, the use of a Graph Neural Network (GNN) could be advantageous.

=== Feed Forward Neural Network
A feedforward neural network is an artificial neural network in which information processing is strictly unidirectional. The input values are multiplied by weighting factors layer by layer and transformed into output values. This model stands in contrast to recurrent neural networks, in which cyclic connections exist that allow information from later processing stages to be fed back into earlier layers.
